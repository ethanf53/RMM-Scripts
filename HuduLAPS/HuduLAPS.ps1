...
if ($company) {	
    ...
    if (($ParentAsset | measure-object).count -eq 1) {
        ...
        If ($ChangeAdminUsername -eq $false) {
            Set-LocalUser -name "Support" -Password ($LocalAdminPassword | ConvertTo-SecureString -AsPlainText -Force) -PasswordNeverExpires:$true
        } else {
            ...
            if (!$ExistingNewAdmin) {
                ...
                Disable-LocalUser -Name "Support"
            } else {
        ...
        if ($ChangeAdminUsername -eq $false ) { $username = "Support" } else { $Username = $NewAdminUsername }


        $PasswordObjectName = "$($Env:COMPUTERNAME) - Local Support Account"
